...
class Connection:
    def __init__(self, password, course_url, email):
        ...
        post_data = urlencode(
            {"email": email, "password": self.pw, "remember": False}
        ).encode("utf-8")
        self.connection = self.get_api_json(
            self.conf["login_page"], post_data
        )  # We can also use API login : /user_api/v1/account/login_session/
        ...
        for cookie_ in self.cookiejar:
            if cookie_.name == "edx-user-info" or cookie_.name == "prod-edx-user-info":
                self.user = json.loads(json.loads(cookie_.value.replace(r"\054", ",")))[
                    "username"
                ]
