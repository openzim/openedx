...
class Video:
    ...
    def download(self, c):
                ...
                for track in subs:
                    if track["src"][0:4] == "http":
                        ...
                        subs_lang[track["srclang"]] = (
                            self.mooc.instance_url + track["src"]
                        )
        else:
            if (
                "fallback" in self.json["student_view_data"]["encoded_videos"]
                and "url"
                in self.json["student_view_data"]["encoded_videos"]["fallback"]
            ):
                url = unquote(
                    self.json["student_view_data"]["encoded_videos"]["fallback"]["url"]
                )
            elif (
                "mobile_low" in self.json["student_view_data"]["encoded_videos"]
                and "url"
                in self.json["student_view_data"]["encoded_videos"]["mobile_low"]
            ):
                url = unquote(
                    self.json["student_view_data"]["encoded_videos"]["mobile_low"][
                        "url"
                    ]
                )
            elif (
                "youtube" in self.json["student_view_data"]["encoded_videos"]
                and "url" in self.json["student_view_data"]["encoded_videos"]["youtube"]
            ):
                ...
                if youtube_json and youtube_json.has_attr("data-metadata"):
                    ...
                    self.no_video = True
                    logging.error(
                        "Cannot get video for {}".format(self.json["lms_web_url"])
                    )
        ...
        self.subs = [
            {"file": os.path.join(self.folder_name, lang + ".vtt"), "code": lang}
